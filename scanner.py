"""
=============================================================================
VN STOCK SCANNER v3.2 — Render 24/7 (NO pandas_ta — tự viết indicator)
=============================================================================
"""
...
# ===========================================================================
# INDICATOR TỰ VIẾT (thay thế pandas_ta)
# ===========================================================================
...
def _bbands(s: pd.Series, n: int = 20):
    """Trả về (lower, mid, upper)."""
    mid   = _sma(s, n)
    std   = s.rolling(n).std()
    return mid - 2 * std, mid, mid + 2 * std

def _bb_width(s: pd.Series, n: int = 20) -> pd.Series:
    lo, mid, hi = _bbands(s, n)
    return (hi - lo) / mid.replace(0, 1e-9)
...
def fetch_ohlcv(ticker: str, start: str, end: str, resolution: str = '1D',
    ...
    if is_intra:
        candidates = [('TCBS', _INTERVAL_TCBS.get(resolution, resolution))]
    elif is_index:
...
def find_bullish_fvg(df: pd.DataFrame, lookback: int = 50) -> list:
    ...
    for i in range(1, len(s)-1):
        ...
        if n['low'] > p['high'] and current >= p['high']*0.97:
            fvgs.append({
                'fvg_top': round(n['low'],2),
                'fvg_bot': round(p['high'],2),
...
def analyze(ticker: str, start_d1: str, start_h1: str, start_m15: str,
    ...
    try:
        ...
        # ── Chỉ báo D1 ───────────────────────────────────────────────────────
        df_d1['ma20']  = _sma(df_d1['close'], 20)
        df_d1['ma50']  = _sma(df_d1['close'], 50)
        df_d1['ema21'] = _ema(df_d1['close'], 21)
        df_d1['bb_w']  = _bb_width(df_d1['close'], 20)
        ...
        mcdx_d1 = calc_mcdx_banker(df_d1, 14)

        # ── Phân tích H1 ─────────────────────────────────────────────────────
        ob_h1 = None; fvg_h1 = []
        struct_h1 = {'bos_bull':False,'choch_bear':False,'hh_hl':False,'last_sh':0,'last_sl':0}
        dry_h1 = False; rsi_h1 = np.nan; ema21_h1 = np.nan

        if df_h1 is not None and len(df_h1) >= 30:
            df_h1['rsi']   = _rsi(df_h1['close'], 14)
            df_h1['ema21'] = _ema(df_h1['close'], 21)
            rsi_h1   = df_h1['rsi'].iloc[-1]
            ema21_h1 = df_h1['ema21'].iloc[-1]
            ob_h1    = find_bullish_ob(df_h1, 60)
            fvg_h1   = find_bullish_fvg(df_h1, 50)
            struct_h1 = detect_structure(df_h1, 40)
            dry_h1   = vol_dryup(df_h1)
        ...
        if df_m15 is not None and len(df_m15) >= 24:
            sweep = detect_sweep(df_m15)
            rev   = detect_reversal(df_m15)
            spike = vol_spike(df_m15)
        ...
        if struct_h1['hh_hl']:
            score += 2; notes.append("✅ HH-HL H1 — uptrend lành mạnh")
        elif struct_h1['bos_bull']:
            score += 1; notes.append("✅ BOS Bullish H1")
        if struct_h1['choch_bear']:
            score -= 2; notes.append("⚠️ CHoCH Bearish H1 — cảnh báo đảo chiều!")
        ...
        for fvg in fvg_h1:
            if fvg['fvg_bot'] <= price <= fvg['fvg_top']:
                score += 2; notes.append(f"🕳️ FVG H1 [{fvg['fvg_bot']}–{fvg['fvg_top']}]"); break
        ...
        if not np.isnan(rsi_h1):
            if trend == 'UPTREND' and 32 <= rsi_h1 <= 52:
                score += 1; notes.append(f"📈 RSI H1 {rsi_h1:.0f} — pullback lý tưởng")
            elif rsi_h1 < 30:
                score += 1; notes.append(f"📉 RSI H1 {rsi_h1:.0f} — oversold")

        if mcdx_d1 > MCDX_THRESH_2:
            score += 2; notes.append(f"💰 MCDX {mcdx_d1:.0f}% — Big Money (+2đ)")
        elif mcdx_d1 > MCDX_THRESH_1:
            score += 1; notes.append(f"💰 MCDX {mcdx_d1:.0f}% — Dòng tiền lớn (+1đ)")

        if rs > 1.10:
            score += 1; notes.append(f"💪 RS {rs:.2f} — mạnh hơn VNI 🔥")
...
# ===========================================================================
# VERDICT — thang điểm nên mua hay không
# ===========================================================================
def build_verdict(res: dict) -> str:
    score = res['score']
    if score >= 9:
        verdict = "🟢 <b>RẤT NÊN MUA</b> — Confluence cực mạnh"
        advice  = "Vào lệnh theo kế hoạch, giữ SL kỷ luật"
    elif score >= 6:
        verdict = "🟢 <b>NÊN MUA</b> — Tín hiệu tốt, đủ confluence"
        advice  = "Mua 50% vị thế, 50% khi có thêm xác nhận"
    elif score >= MIN_SMC_SCORE:
        verdict = "🟡 <b>CÓ THỂ XEM XÉT</b> — Ở mức tối thiểu"
        advice  = "Cỡ lệnh nhỏ (≤30% vốn), chờ thêm tín hiệu"
    elif res['approaching']:
        verdict = "⏳ <b>CHỜ THÊM</b> — Tiệm cận vùng tốt"
        advice  = "Chưa vào lệnh, đặt alert khi giá chạm vùng"
    else:
        verdict = "🔴 <b>KHÔNG NÊN MUA</b> — Chưa đủ điều kiện"
        advice  = "Bỏ qua, tìm mã khác tốt hơn"

    bar_filled = min(score, 10)
    ...
    return (
        ...
        f"   [{bar}]\n"
        f"   <i>Score ≥9 Rất nên | ≥6 Nên | ≥3 Xem xét | &lt;3 Không nên</i>\n"
        f"{'═'*27}\n"
        f"<i>⚠️ Công cụ phân tích kỹ thuật — không phải khuyến nghị đầu tư.</i>"
    )

# ===========================================================================
# ON-DEMAND: Phân tích theo yêu cầu từ Telegram
# ===========================================================================
def analyze_on_demand(ticker: str, chat_id: str):
    with _analysis_lock:
        ticker = ticker.upper().strip()
        log.info(f"[OnDemand] {ticker} → chat {chat_id}")
        send_typing(chat_id)
        send_telegram(
            f"⏳ Đang phân tích <b>{ticker}</b>...\n"
            f"<i>Multi-timeframe D1 + H1 + M15 — chờ ~30 giây</i>",
            chat_id=chat_id
        )
        ...
        if res is None:
            ...
            if df_basic is None:
                send_telegram(
                    f"❌ <b>{ticker}</b> — Không tìm thấy dữ liệu.\n"
                    f"Kiểm tra lại mã CP (VD: ACB, VCB, HPG...)",
                    chat_id=chat_id
                )
            else:
                p = df_basic['close'].iloc[-1]
                send_telegram(
                    f"📊 <b>{ticker}</b>  — Giá đóng cửa: <b>{p:.2f}</b>\n\n"
                    f"🔴 <b>CHƯA ĐỦ ĐIỀU KIỆN</b> vào lệnh lúc này\n\n"
                    f"Lý do có thể:\n"
                    f"• Xu hướng không rõ ràng hoặc DOWNTREND chưa đảo chiều\n"
                    f"• Score SMC/ICT &lt; {MIN_SMC_SCORE} điểm\n"
                    f"• Giá không trong vùng giá trị (OB/OTE/FVG)\n"
                    f"• VN-Index đang {market['regime']} — bộ lọc thị trường từ chối\n\n"
...
# DAILY SCAN
# ===========================================================================
def run_daily_scan(target_chat: str = None):
    chat = target_chat or TELEGRAM_CHAT_ID
    ...
    td = get_last_trading_day()
    send_telegram(
        f"🤖 <b>BÁO CÁO SCANNER v3.2 — {td.strftime('%d/%m/%Y')}</b>\n"
    ...
    def send_group(group, label):
        ...
        for j in range(0, len(group), 3):
            send_telegram("\n\n".join(r['message'] for r in group[j:j+3]), chat_id=chat)
            time.sleep(1.5)
    ...
    if top_a:
        send_group(top_a, f"⏳ VÙNG CHỜ — TOP {len(top_a)} MÃ TIỆM CẬN")
    if not top and not top_a:
        send_telegram(
            "🤖 Không có mã đạt tiêu chuẩn hôm nay.\n"
...
def set_webhook():
    if not WEBHOOK_URL:
        return jsonify({'error': 'WEBHOOK_URL chưa cấu hình'}), 400
...
def webhook():
    try:
        ...
        if text_up in ('/START', 'START'):
            send_telegram(
                "👋 <b>Chào mừng đến VN Stock Scanner v3.2!</b>\n\n"
                "📌 <b>Cách dùng:</b>\n"
                "   Gõ mã CP → nhận phân tích SMC/ICT ngay\n"
                "   VD: <code>ACB</code>  <code>VCB</code>  <code>HPG</code>\n\n"
                "⌨️ <b>Lệnh:</b>\n"
                "   /scan   — Quét toàn bộ watchlist\n"
                "   /status — Trạng thái VN-Index\n"
                "   /help   — Hướng dẫn chi tiết\n\n"
                "<i>⚠️ Công cụ hỗ trợ — không phải khuyến nghị đầu tư.</i>",
                chat_id=chat_id
            )
        ...
        if text_up in ('/HELP', 'HELP'):
            send_telegram(
                "📖 <b>HƯỚNG DẪN</b>\n\n"
                "1️⃣ Gõ mã CP (VD: <code>ACB</code>) → phân tích đa khung\n\n"
                "2️⃣ Kết quả gồm:\n"
                "   • Xu hướng D1 | Score SMC/ICT\n"
                "   • Order Block H1 | FVG H1\n"
                "   • Liquidity Sweep M15 | Nến đảo chiều\n"
                "   • Fibonacci OTE | MCDX Banker\n"
                "   • SL / TP1 / TP2 / R:R cụ thể\n"
                "   • Verdict: Rất nên / Nên / Xem xét / Không nên\n\n"
                "3️⃣ Daily scan tự động 8:15 SA T2–T6\n\n"
                "<i>Score ≥9 Rất nên | ≥6 Nên | ≥3 Xem xét | &lt;3 Không nên</i>",
                chat_id=chat_id
            )
            return jsonify({'ok': True})
        ...
        if text_up in ('/STATUS', 'STATUS'):
            send_telegram("⏳ Đang lấy dữ liệu VN-Index...", chat_id=chat_id)
            def _status():
        ...
        if text_up in ('/SCAN', 'SCAN'):
            send_telegram(
                ...
                f"<i>Mất ~10-15 phút. Kết quả gửi về đây.</i>",
                chat_id=chat_id
            )
            threading.Thread(
        ...
        # Phân tích mã CP
        ticker_match = re.match(r'^/?([A-Z]{2,10})$', text_up)
        if ticker_match:
        ...
        # Không nhận dạng
        send_telegram(
            f"❓ Không hiểu: <code>{raw_text}</code>\n"
            f"Gõ mã CP như <code>ACB</code> hoặc /help",
            chat_id=chat_id
        )
...
def start_scheduler():
    ...
    scheduler.start()
    log.info("✅ Scheduler: Daily scan T2-T6 lúc 08:15 SA (Asia/Ho_Chi_Minh)")
    return scheduler
...
if __name__ == '__main__':
    create_app()
    log.info(f"🚀 VN Stock Scanner v3.2 — port {PORT}")
